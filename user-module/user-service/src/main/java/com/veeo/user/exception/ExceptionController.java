package com.veeo.user.exception;
...
public class ExceptionController {
    ...
    public Result<String> ex(Exception e) {
        log.error("Exception 异常信息 {}", e.getMessage(), e);
    ...
    public Result<String> exception(MethodArgumentNotValidException e) {
        log.error("MethodArgumentNotValidException异常信息 {}", e.getMessage(), e);
        // e.getBindingResult()：获取BindingResult
        BindingResult bindingResult = e.getBindingResult();
        // 收集数据校验失败后的信息
        StringJoiner joiner = new StringJoiner(",");

        bindingResult.getFieldErrors().forEach((fieldError) -> joiner.add(fieldError.getDefaultMessage()));
